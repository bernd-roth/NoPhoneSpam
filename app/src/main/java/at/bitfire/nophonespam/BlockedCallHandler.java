...
public class BlockedCallHandler {
    private static final String TAG = "NoPhoneSpam";
    private static final int NOTIFY_REJECTED = 0;

    @Nullable
    public static Number findBlockedNumber(@NonNull Context context, @Nullable String incomingNumber) {
        if (TextUtils.isEmpty(incomingNumber)) {
            Settings settings = new Settings(context);
            if (settings.blockHiddenNumbers())
                return null; // null means "hidden number matched"
            return null;
        }
    ...
    public static boolean shouldBlock(@NonNull Context context, @Nullable String incomingNumber) {
        if (TextUtils.isEmpty(incomingNumber)) {
            Settings settings = new Settings(context);
    ...
    @Nullable
    public static Number queryAndUpdateDb(@NonNull Context context, @NonNull String incomingNumber) {
        DbHelper dbHelper = new DbHelper(context);
        try {
    ...
    public static void showNotification(@NonNull Context context, @Nullable Number number) {
        Settings settings = new Settings(context);
